...
def part_one():
    with open("./input.txt", 'r', encoding='utf-8') as file:
        hailstones = [Hailstone(*map(int, line.replace("@", ",").split(","))) for line in file.read().splitlines()]
...
def part_two():
    ...
    for i, (sx, sy, sz, vx, vy, vz) in enumerate(hailstones):
        ...
        if i < 2:
            continue
        answers = [soln for soln in solve(equations) if all(x % 1 == 0 for x in soln.values())]
        if len(answers) == 1:
            break
